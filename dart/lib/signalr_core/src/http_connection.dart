...
import 'package:leancode_pipe/signalr_core/src/connection.dart';
import 'package:leancode_pipe/signalr_core/src/http_connection_options.dart';
import 'package:leancode_pipe/signalr_core/src/logger.dart';
import 'package:leancode_pipe/signalr_core/src/transport.dart';
import 'package:leancode_pipe/signalr_core/src/transports/long_polling_transport.dart';
import 'package:leancode_pipe/signalr_core/src/transports/server_sent_events_transport.dart';
import 'package:leancode_pipe/signalr_core/src/transports/web_socket_transport.dart';
...
class HttpConnection implements Connection {
  ...
  void _stopConnection({Exception? exception}) {
    ...
    final stopReason = (_stopException == null) ? exception : _stopException;
    _stopException = null;

    if (_connectionState == ConnectionState.disconnected) {
      _logging!(
        LogLevel.debug,
        'Call to HttpConnection.stopConnection(${stopReason.toString()}) was '
    ...
    if (_connectionState == ConnectionState.connecting) {
      ...
      throw Exception(
        'HttpConnection.stopConnection(${stopReason.toString()}) was called '
    ...
    if (stopReason != null) {
      _logging!(LogLevel.error,
          'Connection disconnected with error \'${stopReason.toString()}\'.');
    } else {
    ...
    if (_connectionStarted) {
      ...
      try {
        ...
        _logging!(
          LogLevel.error,
          'HttpConnection.onclose(${stopReason.toString()}) threw '
          'error \'${e.toString()}\'.',
        );
      }
    }
