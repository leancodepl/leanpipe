...
import { fromEvent, Observable, ReplaySubject, throwError } from "rxjs"
    ...
    this.#connection$ = new Observable<HubConnection>(subscriber => {
      const connection = new HubConnectionBuilder()
        ...
        .withAutomaticReconnect({
          nextRetryDelayInMilliseconds: ({ previousRetryCount }) => [0, 500, 1000].at(previousRetryCount) ?? 2000,
        })
        .build()
        ...
        notifications$: this.#connection$.pipe(
          switchMap(({ connection, notifications$, subscriptionResults$ }) => {
            ...
            return subscriptionResults$.pipe(
              first<SubscriptionResult>(matches({ SubscriptionId: subscriptionId, Type: OperationType.Subscribe })),
              timeout({ first: 3000 }),
              switchMap(({ Status }) =>
