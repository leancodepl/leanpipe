        ...
        ITopicController<Auction, ItemSold>
{
    ...
    public Task<IEnumerable<string>> ToKeysAsync(Auction topic, BidPlaced notification, LeanPipeContext context) =>
        Task.FromResult(ToKeys(topic));

    public Task<IEnumerable<string>> ToKeysAsync(Auction topic, ItemSold notification, LeanPipeContext context) =>
        Task.FromResult(ToKeys(topic));
}
